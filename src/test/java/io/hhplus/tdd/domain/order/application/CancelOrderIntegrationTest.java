...
class CancelOrderIntegrationTest extends BaseIntegrationTest {
    ...
    @Test
    @DisplayName("주문 취소 통합 테스트 - 결제 완료 후 취소는 불가능")
    void 결제_완료_후_주문_취소_불가() {
        // given
        UserPoint userPoint = UserPoint.builder()
                .balance(100000L)
                .build();
        ...
        // when & then
        CancelOrderUseCase.Input cancelInput = new CancelOrderUseCase.Input(savedOrder.getId());
        ...
        // 주문 상태가 여전히 PAID인지 확인
        Order unchangedOrder = orderRepository.findById(savedOrder.getId()).orElseThrow();
