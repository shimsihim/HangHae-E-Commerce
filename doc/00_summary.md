...
|-------|---------------------|-------------------|-------------------------------|
| 회원    | users              | 유저 정보 및 포인트       | ✅ (balance - 낙관적 락)          |
| 회원    | point_history      | 포인트 충전/사용 이력      | X                             |
...
- `users.balance`: 포인트 충전/사용 시 버전 관리
...
### 테이블 구조 간소화
동시성 제어가 필요한 필드를 별도 테이블로 수직 분할하지 않고 메인 테이블에 통합하여 구조를 간소화했습니다.
- ~~user_point~~ → `users` 테이블에 balance, version 포함
- ~~product_option_stock~~ → `product_options` 테이블에 quantity 포함
- ~~coupon_metas, coupon_issue_policy~~ → `coupons` 테이블로 통합
