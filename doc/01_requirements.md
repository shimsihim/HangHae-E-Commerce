...
**상세 설명**
- 이메일(아이디)과 비밀번호를 입력받아 인증 수행
- 인증 성공 시 JWT 토큰 또는 세션 ID 발급
- 비밀번호는 암호화하여 저장 (BCrypt 등)
...
**상세 설명**
- 포인트 충전/사용 시 users 테이블의 version 필드를 활용한 낙관적 락 적용
...
3. **포인트 차감**: users.balance에서 최종 결제 금액 차감 (낙관적 락)
